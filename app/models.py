...
news_info = Table(
    ...
    Column("news_date", Date, nullable=False, server_default=func.current_date()),
    Column("data", JSONB),
    Column("created_at", TIMESTAMP(timezone=True), nullable=False, server_default=func.current_timestamp()),
    Column("updated_at", TIMESTAMP(timezone=True), nullable=False, server_default=func.current_timestamp()),
    Column("extracted", Boolean, server_default="false"),
...
news_item = Table(
    ...
    Column("created_at", TIMESTAMP(timezone=True), nullable=True, server_default=func.current_timestamp()),
    Column("updated_at", TIMESTAMP(timezone=True), nullable=True, server_default=func.current_timestamp()),
...
news_event = Table(
    ...
    Column("cluster_id", BigInteger, nullable=False),
    Column("title", Text),
    Column("summary", Text),
...
news_event_item = Table(
    "news_event_item",
    metadata,
    Column("event_id", BigInteger, ForeignKey("news_event.id", ondelete="CASCADE"), nullable=False, primary_key=True),
    Column("news_id", BigInteger, ForeignKey("news_item.id", ondelete="CASCADE"), nullable=False, primary_key=True),
)


news_event_pipeline_run = Table(
    "news_event_pipeline_run",
    metadata,
    Column("id", BigInteger, primary_key=True, autoincrement=True),
